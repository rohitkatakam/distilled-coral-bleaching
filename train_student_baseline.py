"""CLI entry point to train the student baseline model for coral bleaching classification."""
...
def parse_args(args=None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train the student baseline model.")
    parser.add_argument("--config", type=str, default="configs/config.yaml", help="Path to the training config.")
    parser.add_argument("--output-dir", type=str, default="checkpoints/student_baseline", help="Directory to store student baseline checkpoints.")
...
def setup_wandb(args: argparse.Namespace, config: dict, run_name: str = None):
    ...
    wandb.init(
        ...
        name=run_name or "student-baseline-training",
        config=wandb_config,
...
def main(args=None) -> None:
    """Execute student baseline training workflow."""
    # Parse arguments
    if args is None:
        args = parse_args()
    ...
    # Initialize model
    print("\nInitializing student baseline model...")
    use_pretrained = not args.no_pretrained
    model = StudentModel(num_classes=2, pretrained=use_pretrained)
