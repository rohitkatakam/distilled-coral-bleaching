"""Tests for teacher training script."""
...
class TestArgumentParsing:
    ...
    def test_default_args(self):
        ...
        assert args.output_dir == "checkpoints/teacher"
...
class TestCheckpointSaveLoad:
    ...
    def test_load_checkpoint(self, model, optimizer, scheduler):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            new_model = TeacherModel(num_classes=2, pretrained=False)
            ...
            original_weights = model.backbone.conv1.weight.data
            loaded_weights = new_model.backbone.conv1.weight.data
...
class TestTrainingOneEpoch:
    ...
    def test_train_one_epoch_updates_weights(self, model, train_loader):
        ...
        # Get initial weights
        initial_weights = model.backbone.fc.weight.data.clone()

        # Train for one epoch
        train_one_epoch(model, train_loader, criterion, optimizer, device, epoch=1)

        # Get updated weights
        updated_weights = model.backbone.fc.weight.data
...
class TestValidation:
    ...
    def test_validate_no_gradient_computation(self, model, val_loader):
    ...
    def temp_config_path(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            config_data = {
                ...
                'model': {
                    'teacher': {'name': 'resnet50', 'pretrained': False}
                },
                'training': {
                    'device': 'cpu',
                    'epochs': 2,
